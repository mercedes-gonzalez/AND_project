...
img = dip.im_read(join(root_path,filename)).astype('float32')
# img = dip.imread('C:/Users/mgonzalez91/PycharmProjects/Images/cameraman.tif').astype('float32')
imgsize = img.shape
M = imgsize[0]
N = imgsize[1]
...
sigma_color = 100   # Filter sigma in the color space. A larger value of the parameter means that farther colors within the pixel 
...
threshold_perc = .6 # pixel intensity threshold as percentage (quantiles)
...
_, gaussian_mask = cv2.threshold(gaussian_blur,np.quantile(img,threshold_perc),1,cv2.THRESH_TOZERO)
_, median_mask = cv2.threshold(median_blur,np.quantile(img,threshold_perc),1,cv2.THRESH_TOZERO)
_, bilateral_mask = cv2.threshold(bilateral_blur,np.quantile(img,threshold_perc),1,cv2.THRESH_TOZERO)
...
# --- Contouring  ---------------------------------------------------------------------------
gaussian_contour, _ = cv2.findContours(gaussian_mask.astype('uint8'),cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_NONE)
median_contour, _ = cv2.findContours(median_mask.astype('uint8'),cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_NONE)
bilateral_contour, _ = cv2.findContours(bilateral_mask.astype('uint8'),cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_NONE)
...
for cnt in contourlist:
    ...
    # if cv2.contourArea(cnt) < .01*row*col and cv2.contourArea(cnt) > .0005*row*col and cv2.contourArea(cnt) > (radius*radius*3.1415*.5):
        cv2.drawContours(contour, cnt, -1, (1,1,1), 3)
